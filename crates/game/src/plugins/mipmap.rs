...
use bevy::{
    prelude::*,
    render::{
        render_resource::{SamplerDescriptor, TextureDimension, TextureFormat},
...
pub fn generate_mips_texture(image: &mut Image, settings: &MipmapGeneratorSettings) -> anyhow::Result<()> {
    check_image_compatible(image)?;
    match try_into_dynamic(image.clone()) {
        Ok(mut dyn_image) => {
            let (mip_level_count, image_data) =
                generate_mips(&mut dyn_image, settings.minimum_mip_resolution, u32::MAX, settings.filter_type);
...
pub fn generate_mips(
    ...
    while width / 2 >= minimum_mip_resolution.max(1) && height / 2 >= minimum_mip_resolution.max(1) && mip_level_count < max_mip_count {
        width /= 2;
        height /= 2;
...
pub fn try_into_dynamic(image: Image) -> anyhow::Result<DynamicImage> {
    match image.texture_descriptor.format {
        ...
        texture_format => return Err(anyhow!("Conversion into dynamic image not supported for {:?}.", texture_format)),
    }
    .ok_or_else(|| anyhow!("Failed to convert into {:?}.", image.texture_descriptor.format))
}
